...
if [ -f run.sh ] && [ -f md5 ]; then
	...
	cd ..
	tar -cvf NEW-PDA-boot.img.tar boot.img && mv boot.img kernel/ && cd kernel && zip -r ../NEW-boot.img.zip . ;
	cd ..
	mv NEW-boot.img.zip ../$(cat ../common/.version)-NEW-boot.img.zip && echo "$(cat ../common/.version)-NEW-boot.img.zip <-> READY"
	mv NEW-PDA-boot.img.tar ../$(cat ../common/.version)-NEW-PDA-boot.img.tar && echo "$(cat ../common/.version)-NEW-PDA-boot.img.tar <-> READY"
else
