...
if [ -f boot.img-ramdisk.cpio.gz ] || [ -f boot.img-ramdisk.cpio.lzma ] || [ -f boot.img-ramdisk.cpio.lzo ]; then
	...
	rm -Rf ramdisk
	ln -s ramdisk_REWRITTEN ramdisk
	if [ -d ramdisk ]; then
...
if [ -f run.sh ] && [ -f md5 ]; then
	...
	cd ..
	tar -cvf REWRITTEN-PDA-boot.img.tar boot.img && mv boot.img kernel/ && cd kernel && zip -r ../REWRITTEN-boot.img.zip . ;
	cd ..
	mv REWRITTEN-boot.img.zip ../$(cat ../common/.version)-REWRITTEN-boot.img.zip && echo "$(cat ../common/.version)-REWRITTEN-boot.img.zip <-> READY"
	mv REWRITTEN-PDA-boot.img.tar ../$(cat ../common/.version)-REWRITTEN-PDA-boot.img.tar && echo "$(cat ../common/.version)-REWRITTEN-PDA-boot.img.tar <-> READY"
else
